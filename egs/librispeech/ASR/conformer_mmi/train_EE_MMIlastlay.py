...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 18 21:57:27 2023
...
def compute_loss(
    ...
    with torch.set_grad_enabled(is_training):
        ...
        for i in range(len(nnet_output)):
            ...
            mmi_loss_int = loss_fn(dense_fsa_vec=dense_fsa_vec, texts=texts,is_last_layer=True)
    ...
    return losses_list, loss, mmi_loss.detach(), att_loss.detach()
...
def compute_validation_loss(
    ...
    loss_0 = loss_1 = loss_2 = loss_3 = loss_4 = loss_5 = 0.
    
    
    for batch_idx, batch in enumerate(valid_dl):
        
        losses_list, loss, mmi_loss, att_loss = compute_loss(
            params=params,
            model=model,
            batch=batch,
            graph_compiler=graph_compiler,
        ...
        loss_0 += losses_list[0].detach().cpu().item(); loss_1 += losses_list[1].detach().cpu().item(); loss_2 += losses_list[2].detach().cpu().item()
        loss_3 += losses_list[3].detach().cpu().item(); loss_4 += losses_list[4].detach().cpu().item(); loss_5 += losses_list[5].detach().cpu().item()
    ...
    if world_size > 1:
        s = torch.tensor(
            [tot_loss, tot_mmi_loss, tot_att_loss, tot_frames, loss_0, loss_1, loss_2, loss_3, loss_4, loss_5],
            device=loss.device
        )
        ...
        loss_0 = s[4]; loss_1 = s[5]; loss_2 = s[6]; loss_3 = s[7]; loss_4 = s[8]; loss_5 = s[9];
    
    
    params.val_loss_0 = loss_0 / tot_frames; params.val_loss_1 = loss_1 / tot_frames; params.val_loss_2 = loss_2 / tot_frames; params.val_loss_3 = loss_3 / tot_frames; params.val_loss_4 = loss_4 / tot_frames; params.val_loss_5 = loss_5 / tot_frames;   
...
def train_one_epoch(
    ...
    loss_0_t = loss_1_t = loss_2_t = loss_3_t = loss_4_t = loss_5_t = 0.
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        losses_list_t, loss, mmi_loss, att_loss = compute_loss(
            params=params,
            model=model,
            batch=batch,
            graph_compiler=graph_compiler,
        ...
        loss_0_t += losses_list_t[0].detach().cpu().item(); loss_1_t += losses_list_t[1].detach().cpu().item(); loss_2_t += losses_list_t[2].detach().cpu().item()
        loss_3_t += losses_list_t[3].detach().cpu().item(); loss_4_t += losses_list_t[4].detach().cpu().item(); loss_5_t += losses_list_t[5].detach().cpu().item()
        ...
        params.train_loss_0 = loss_0_t / tot_frames; params.train_loss_1 = loss_1_t / tot_frames; params.train_loss_2 = loss_2_t / tot_frames; params.train_loss_3 = loss_3_t / tot_frames; params.train_loss_4 = loss_4_t / tot_frames; params.train_loss_5 = loss_5_t / tot_frames;   
