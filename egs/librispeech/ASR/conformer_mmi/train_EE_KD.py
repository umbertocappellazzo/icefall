...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul 27 15:50:44 2023
...
def load_checkpoint_if_available(
    ...
    if params.start_epoch <= 0:
        return
    filename = Path('conformer_mmi/exp_EE') / f"epoch-{params.start_epoch-1}.pt"
...
def compute_loss(
    ...
    return loss, mmi_loss.detach(), mse_loss.detach(), att_loss.detach()
...
def compute_validation_loss(
    ...
    for batch_idx, batch in enumerate(valid_dl):
        loss, mmi_loss, mse_loss, att_loss = compute_loss(
            params=params,
            model=model,
            batch=batch,
            graph_compiler=graph_compiler,
        ...
        tot_mse_loss += mse_loss.detach().cpu().item()

        tot_frames += params.valid_frames

    if world_size > 1:
        s = torch.tensor(
            [tot_loss, tot_mmi_loss, tot_mse_loss, tot_att_loss, tot_frames],
            device=loss.device,
        )
    ...
    params.valid_mse_loss = tot_mse_loss / tot_frames
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        tot_avg_mse_loss = tot_mse_loss / tot_frames
        ...
        if batch_idx > 0 and batch_idx % params.valid_interval == 0:
            ...
            if tb_writer is not None:
                ...
                tb_writer.add_scalar(
                    "train/valid_mse_loss",
                    params.valid_mse_loss,
                    params.batch_idx_train,
                )
